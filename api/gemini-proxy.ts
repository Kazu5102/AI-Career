...
あなたのタスクは、**対話が自然な区切りを迎えているか**を判断することです。特に、AI自身の最後の返答が、話を締めくくるような内容（例：「何か他に質問はありますか？」）や、ユーザーに次の行動を促すような内容だった場合、それは提案の絶好のタイミングです。

もし、そのようなタイミングだと判断した場合、ユーザーが会話をさらに深掘りしたり、別の角度から考えたりするのに役立つ、簡潔で具体的な質問を3つ提案してください。提案する質問は、これまでの会話の流れに沿ったもので、ユーザーが自己分析を続けられるような内容にしてください。

逆に、AIが具体的な質問を投げかけた直後で、ユーザーの返答を待っている状態の場合は、提案は不要です。その場合は空の配列を返してください。
